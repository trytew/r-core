...
use embedded_graphics::prelude::{
    DrawTarget, OriginDimensions, PixelColor, Point, Primitive, RgbColor, Size,
};
use embedded_graphics::primitives::{PrimitiveStyle, Rectangle};
use embedded_graphics::{Drawable, Pixel};
use user_lib::console::getchar;
use user_lib::{key_pressed, sleep, Display, VIRT_GPU_X_RES, VIRT_GPU_Y_RES};
...
impl<'a, T: DrawTarget> OriginDimensions for ScaledDisplay<'a, T> {
    fn size(&self) -> Size {
        Size::new(self.size_x, self.size_y)
    }
}
...
pub fn main() -> i32 {
    ...
    let _ = display.clear(Rgb888::BLACK).unwrap();
    loop {
        if key_pressed() {
            let c = getchar();
            match c {
